    ...
    try:
        ...
        try:
            ...
            with _fits.open(filepath) as _hdul:
                try:
                    ...
                    _raw = _hdul[0].data
                nan_mask = np.isnan(_raw) if _raw is not None else None
        except Exception:
            nan_mask = None
        ...
        try:
            ...
            celestial_objects = sh.extract_objects(bkgless_data, bkg, 
                                                   ...
                                                   pixel_stack_limit=5000000, sub_object_limit=50000)
        except MemoryError:
        ...
        for i, obj in enumerate(celestial_objects):
            ...
            if mode == 'crop-only':
                ...
                obj_npix = int(obj['npix']) if 'npix' in obj.dtype.names else None
                obj_elongation = float(obj['a'] / obj['b']) if ('a' in obj.dtype.names and 'b' in obj.dtype.names and obj['b'] > 0) else None
                # SNR: flux / (bkg_rms * sqrt(npix))
                obj_snr = None
                if obj_flux is not None and obj_npix is not None and obj_npix > 0:
                    try:
                        bkg_rms_local = sep.Background(image_data).globalrms
                        obj_snr = obj_flux / (bkg_rms_local * np.sqrt(obj_npix)) if bkg_rms_local > 0 else None
                    except Exception:
                        pass
