...
for row in sample_data[["name", "predicted_spirality"]].itertuples(index=False):
    ...
    for image_path in image_directory.rglob(file+".fits"):
        ...
        new_filepath = str(output_directory.parent) + "/" + output_directory.name + "/" + str(p_spiral)[:10] + "_" + file +".png"
        sh.save_fits_as_png(fits_data, new_filepath)
        
        copied_files += 1
        break
    
    # for image_path in image_directory.rglob(file+"-J_logSpiralArcs-merged.png"):
    #     print("found file")
    #     new_filepath = str(output_directory.parent) + "/" + output_directory.name + "/" + str(p_spiral)[:10] + "_" + file +".png"
