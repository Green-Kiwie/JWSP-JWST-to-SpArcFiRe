...
def load_training_data(data_path: Path = "randomforest_training_data/data_cleaned.csv") -> pd.DataFrame:
    ...
    main_data = pd.read_csv(data_path, dtype=dtype)
...
def load_inference_data(data_path: Path, training_data_filepath: Path =  "randomforest_training_data/data_cleaned.csv") -> pd.DataFrame:
    """
    Loads data with columns based on training data column types
    """
    training_data = load_training_data(training_data_filepath)

    dtype_map = training_data.dtypes.to_dict()
    dtype_map = {
        col: str(dtype) for col, dtype in dtype_map.items()
        if str(dtype) in ['int64', 'float64', 'object', 'bool', 'string']
    }
    
    main_data = pd.read_csv(data_path, dtype=dtype_map)
